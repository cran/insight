...
test_that("cplm, tweedie", {
  ...
  mu <- exp(
    cplm::fixef(parmodCPr) +
      0.5 *
        (cplm::VarCorr(parmodCPr)$Population[1] + cplm::VarCorr(parmodCPr)$Container[1])
  )
  ...
  R2glmmC <- (VarF + sum(as.numeric(cplm::VarCorr(parmodCPf)))) /
    (VarF + sum(as.numeric(cplm::VarCorr(parmodCPf))) + VarOlF)
  ...
  R2glmmC <- (VarF + sum(as.numeric(cplm::VarCorr(parmodCPf)))) /
    (VarF + sum(as.numeric(cplm::VarCorr(parmodCPf))) + VarOdF)
  out2 <- performance::r2_nakagawa(
    parmodCPf,
    null_model = parmodCPr,
    approximation = "delta"
  )
